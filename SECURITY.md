...
- The optional Session Recorder uses a separate `127.0.0.1` server with exact Host/Origin validation, no CORS, authenticated GET-only APIs, and a strict local-only CSP. Its random bearer token is delivered in the URL fragment, removed immediately, and retained only in tab memory. Do not bind or reverse-proxy this surface to a network interface.
- Dashboard history is disabled unless a dashboard session is requested. Enabled sessions persist allowlisted metadata under `~/.pinpoint/dashboard` using mode-0700 directories and mode-0600 files, with 30-day/64 MiB bounds. Processes running as the same OS user may still read those files or process state; loopback and file modes are not an OS sandbox.
